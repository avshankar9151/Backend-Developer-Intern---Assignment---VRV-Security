...
class UserNewAdmin(UserAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if form.base_fields.get("user_permissions") and not request.user.is_superuser:
            ...
            limited_permissions = Permission.objects.filter(
                Q(content_type__app_label__in=["rbac"])
                | Q(content_type__model__in=["group"])
            ).exclude(Q(codename__in=["delete_group"]))
            form.base_fields["user_permissions"].queryset = limited_permissions
        return form
